  ...
  <v-card>
    ...
    <v-simple-table>
      <tbody>
        ...
        <tr>
          <th>Tag/Service/Generated/Mark&nbsp;filter</th>
          <td><code>tag:tagname,othertag</code>, <code>service:svc</code>, <code>mark:marked</code> or <code>generated:foo</code></td>
          <td width="100%">
        ...
        <tr>
          ...
          <td width="100%">
            Restricts the results to only streams with one of the given ids. You
            can give a list of (separate by <code>,</code>) ids or id ranges
            (using <code>:</code>), id ranges can be open(by leaving out the
            number) at any side. Any of these variables, optionally from
            subqueries, can be used: <code>id</code>, <code>[cs]port</code>,
            <code>[cs]bytes</code>. Simple calculations can be performed, using
        ...
        <tr>
          ...
          <td width="100%">
            <code>cport</code>, <code>sport</code> and <code>port</code> filter
            on the client, server or any port. The syntax is identical to the
        ...
        <tr>
          ...
          <td width="100%">
            ...
            <code>bytes</code> filter on the number of bytes send by the client,
            server or any of them. The syntax is identical to the
            <code>id</code> filter syntax.
          </td>
        </tr>
        <tr>
          ...
          <td width="100%">
            <code>chost</code>, <code>shost</code> and <code>host</code> filter
            on the client, server or any host, lists are supported, each entry
            consists of an ip-address or a variable (e.g.
            <code>@subquery:[cs]host@</code>). Optionally, one or more
        ...
        <tr>
          ...
          <td width="100%">
            ...
            (e.g. <code>-2h3m4s</code>) or an absolute time using the format
            <code>[YYYY-MM-DD ]HHMM[SS]</code>. <code>[fl]time</code> variables
            can be used as well as simple calculations using <code>+</code> and
            <code>-</code>. For finding streams that lasted 5 minutes or longer
            you could e.g. use <code>ltime:@ftime@+5m</code>.
          </td>
        </tr>
        <tr>
          <th>Data&nbsp;filter</th>
          ...
          <td width="100%">
            Select streams that contain the given regex in the data send by the
            client(<code>cdata</code>), server(<code>sdata</code>) or any of
            ...
            <code
              ...
              cdata:"@flag@"</code
            >. One set of <code>then</code>-connected <code>data</code> filters
            must belong to the same sub-query. A data filter can reference
            variables generated by sub queries that are
            <code>and</code> connected. E.g.
            <code
        ...
        <tr>
          ...
          <td width="100%">
            ...
            only accepts a number as value, the default is <code>100</code>, the
            value <code>0</code> means unlimited.
          </td>
        </tr>
        <tr>
          <th>Grouping</th>
