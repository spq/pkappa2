...
export default function suggest(query, cursorOffset, groupedTags, converters) {
  ...
  if (!targetElem) return { suggestions: [] };
  ...
  if (
    ...
    const suggestions = tagsInGroup.filter(
      (t) => null == value || (t.startsWith(value) && t !== value)
    );
    return {
      suggestions,
      start,
    ...
    const suggestions = converters
      .filter(
        (c) => null == value || (c.Name.startsWith(value) && c.Name !== value)
      )
      .map((c) => c.Name);
    return {
      suggestions,
...
function _findElementAtCursor(results, cursorOffset) {
  ...
  while (elements.length > 0) {
    ...
    if (elem["type"] === "expression") {
      if (elem["value"] && isCursorInsideElement(elem, "value")) return elem;
